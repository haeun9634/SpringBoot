...
![***이렇게 로그 출력이 되시면 성공입니다!***](image%205.png)
...
- N+1 문제를 해결할 수 있는 여러 가지 다른 방법
    ...
    ### **N+1 문제를 해결할 수 있는 여러 가지 방법**
    
    ### **1. `@Query` 또는 `JPQL`을 사용한 `JOIN FETCH`**
    
    - **`JOIN FETCH`**를 사용하면 연관된 엔티티들을 한 번의 쿼리로 함께 조회
    - 특히 **`OneToMany`** 관계에서 유용
    
    예시:
    
    ```java
    @Query("SELECT p FROM Parent p JOIN FETCH p.children WHERE p.id = :parentId")
    Parent findParentWithChildren(@Param("parentId") Long parentId);
    
    ```
    
    - **`JOIN FETCH`**는 **`Parent`** 엔티티와 연관된 **`Children`** 엔티티를 한 번의 쿼리로 가져옴
    - 이 방법은 **`fetch`** 조인으로, 여러 개의 쿼리를 실행하는 것보다 성능이 훨씬 더 나음
    
    ### **2. `@EntityGraph` 사용**
    
    - **`@EntityGraph`**는 **`JPA`**에서 제공하는 또 다른 방법으로, 연관된 엔티티를 한 번의 쿼리로 가져올 수 있게 함.
    - **`@Query`**를 사용한 방법과 유사하지만, 코드에서 **`fetch`** 전략을 직접 지정 가능
    ...
    - **`@EntityGraph`**는 연관된 엔티티를 명시적으로 **`fetch`**하는 기능을 제공하며, 필요한 필드만 지정하여 성능을 최적화
    - **`@EntityGraph`**는 **`JOIN FETCH`**와 유사한 기능을 하지만, 쿼리 내에서 명시적으로 연관된 속성만 로딩하도록 제어 가능
    
    ### **3. `@OneToMany` 또는 `@ManyToOne` 관계에서 `LAZY`로 설정하고`@Transactional` 범위 내에서만 사용**
    
    - **`LAZY`** 로딩을 사용할 때, 연관된 엔티티를 실제로 사용할 때만 조회하도록 설정
    ...
    - **`LAZY`** 로딩을 사용하면 관련된 데이터를 즉시 로드하지 않기 때문에 N+1 문제를 피할 수 있다.
    - 하지만 **`LAZY`** 로딩을 사용할 때는 **`@Transactional`** 범위 내에서 사용해야만 세션을 열어두고 조회할 수 있다.
    
    ### **4. `@BatchSize` 사용 (배치 사이즈 설정)**
    
    - **`@BatchSize`**는 한 번에 여러 개의 연관된 엔티티를 조회하도록 설정할 수 있는 방법. 연관된 엔티티들을 배치로 묶어서 한 번에 조회한다.
    ...
    - 위 예제에서는 **`children`** 리스트를 조회할 때 한 번에 최대 10개의 **`Child`** 엔티티를 조회
    - **`@BatchSize`**를 사용하면 여러 개의 쿼리가 실행될 때, 각 쿼리에서 조회할 엔티티 수를 제어하여 성능을 최적화 가능
    
    ### **5. `JPA`의 `@Fetch(FetchMode.SUBSELECT)` 사용**
    
    - **`FetchMode.SUBSELECT`**는 연관된 엔티티들을 서브쿼리로 한번에 조회하는 방법. 여러 개의 **`SELECT`** 쿼리로 데이터를 조회하는 대신 하나의 서브쿼리로 조회할 수 있음
    ...
    - **`FetchMode.SUBSELECT`**는 연관된 엔티티를 서브쿼리로 가져와 한 번에 조회할 수 있도록 하여 N+1 문제를 피할 수 있음
    - 이 방법은 성능을 최적화할 수 있지만, 특정 쿼리에서는 서브쿼리가 너무 복잡해질 수 있기 때문에 사용에 주의가 필요
    
    ### **6. `QueryDSL`에서 `fetchJoin` 사용**
    
    - **`QueryDSL`**에서도 **`fetchJoin`**을 사용하여 연관된 엔티티를 조인하여 한번에 로딩 가능
    ...
    JPAQuery<?> query = jpaQueryFactory.selectFrom(parent)
            .leftJoin(parent.children, child).fetchJoin()
            .where(parent.id.eq(parentId));
    
    Parent parent = query.fetchOne();
    
    ```
    
    - **`fetchJoin()`**은 **`JOIN FETCH`**와 동일한 역할을 하며, 연관된 엔티티를 한 번의 쿼리로 로딩
    - **`QueryDSL`**을 사용할 경우에도 이 방법을 통해 성능 문제를 해결할 수 있음
    
    ---
    
    ### 정리
    
    - **`JOIN FETCH`** 또는 **`@EntityGraph`**를 사용하여 연관된 데이터를 한 번에 로딩.
    - **`LAZY`** 로딩을 사용하고 **`@Transactional`** 내에서만 데이터 조회.
    - **`@BatchSize`**로 한 번에 조회할 엔티티 수를 설정하여 배치 처리를 최적화.
    - **`FetchMode.SUBSELECT`**와 같은 서브쿼리 방식을 사용하여 조회 최적화.
...
- **미션 기록**
    ...
    [2주차 실전 SQL](https://www.notion.so/2-SQL-669cbd5c62ca4b9c9c43305059482259?pvs=21)
    
    [미션 관련 쿼리 정리](https://www.notion.so/d46afa68528545bf8bab38e3b33d4975?pvs=21)
    
    [리뷰 관련 쿼리 정리](https://www.notion.so/aed4bee1fa304857bcf2e359ca940e54?pvs=21)
    
    [사용자 관련 쿼리 정리](https://www.notion.so/597fd8f7885f4da7846a239273d72a71?pvs=21)
    
    [알림 관련 쿼리 정리](https://www.notion.so/23db3b1fbc9848518f676904e29ca95b?pvs=21)
    

[테스트 시작](https://www.notion.so/6af654741705470cb36ece36b4775d91?pvs=21)

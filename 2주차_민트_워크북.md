...
![마이 페이지 화면 쿼리4](Untitled%207.png)

마이 페이지 화면 쿼리4

### 1주차 때 설계한 DB

![MissonUMC.png](MissonUMC.png)

### 진행중인 미션 모아보는 쿼리

데드라인 기준.  커서페이징

필요한 값: **user_id**, **마지막으로 조회한 미션의 id** 

마지막으로 조회한 미션 마감일보다 느린 (>) 값들을 조회하도록 설정

```sql
AND m.deadline_at > (SELECT deadline_at FROM Misson WHERE id = ?) 
```
...
```sql
SELECT * FROM Misson AS m
INNER JOIN Misson_table AS mt ON m.id = mt.mission_id 
WHERE mt.user_id = ?
...
WHERE mt.user_id = ?
AND m.complete = true
AND m.update_at > (SELECT update_at FROM Misson WHERE id = ?) 
...
리뷰 작성이므로 값 추가하는 insert문 작성. 

필요한 값: 유저id, 작성할 가게의 id, 리뷰 내용, 사진 url, 평점

```sql
INSERT INTO Review (user_id, store_id, content, photo, score) VALUES (?, ?, ?, ?, ?);
```

### 홈화면 쿼리

현재 달성한 미션의 갯수를 나타내는 쿼리

```sql
SELECT count(*) FROM Misson AS m
INNER JOIN Misson_table AS mt ON m.id = mt.mission_id 
WHERE mt.user_id = ?
WHERE User.id=? AND m.complete=true;
```

현재 선택 된 지역에서 도전이 가능한 미션 목록

먼저 사용자가 선택한 지역에서의 지도 ID를 찾고 해당 지도 ID에 선택되어있는 가게 id를 출력

```sql
(SELECT map.store_id FROM Map AS map
INNER JOIN Map_table AS mt ON map.id = mt.map_id 
WHERE mt.user_id = ?);
```

1. user_id를 지도 매핑 테이블에서 찾아서 조회 
2. 지도 매핑 테이블의 지도 id와 지도 테이블의 id가 같은 경우를 조회
3. 지도 테이블의 id에 맞는 가게 id를 조회

따라서 지도 매핑 테이블에서 user_id와 맞는 map_id들을 찾고, map_id는 지도 테이블에서 **가게 id를 찾아서 출력하는 코드**
...
```sql
SELECT * FROM Mission AS m WHERE m.store_id = (SELECT map.store_id FROM Map AS map
INNER JOIN Map_table AS mt ON map.id = mt.map_id 
WHERE mt.user_id = ?);
```

**마지막으로 조회한 커서 id를 받아와 커서 페이징 조회**

```python
SELECT * FROM Mission AS m WHERE m.store_id = (
    SELECT map.store_id 
    FROM Map AS map
    INNER JOIN Map_table AS mt ON map.id = mt.map_id 
    WHERE mt.user_id = ?)
...
유저 닉네임, 이메일, 휴대폰 번호 가져오는 쿼리

```sql
SELECT nickname, email, phone, point FROM User WHERE user_id = ?;
```

사용자가 작성한 리뷰 가져오는 쿼리

```sql
SELECT * FROM Reivew WHERE user_id = ?;
```

알림 동의 on

```sql
UPDATE Alerm SET agree=true where user_id = ?
```
...
[2주차](2%E1%84%8C%E1%85%AE%E1%84%8E%E1%85%A1%20fffb57f4596b81baafe5d9a6db38f874.csv)
